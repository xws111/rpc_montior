...
namespace monitor {
void CpuSoftIrqMonitor::update_once(rpc_monitor::MonitorInfo* monitor_info) {
  FileReader softirqs_file("/tmp_proc/softirqs");
  std::vector<std::string> one_softirq;
  std::vector<std::vector<std::string>> softirq;
  while (softirqs_file.read_line(&one_softirq)) {
