...
namespace monitor {
CpuLoadModel::CpuLoadModel(QObject* parent) : MonitorModelBase(parent) {
  header_ << tr("load_1");
  header_ << tr("load_3");
  header_ << tr("load_15");
}
...
QVariant CpuLoadModel::headerData(int section, Qt::Orientation orientation,
  ...
  return MonitorModelBase::headerData(section, orientation, role);
}
...
QVariant CpuLoadModel::data(const QModelIndex& index, int role) const {
  ...
  return MonitorModelBase::data(index, role);
}

void CpuLoadModel::UpdateMonitorInfo(
    const rpc_monitor::MonitorInfo& monitor_info) {
  beginResetModel();
  monitor_data_.clear();
...
std::vector<QVariant> CpuLoadModel::insert_one_cpu_load(
    const rpc_monitor::CpuLoad& cpu_load) {
